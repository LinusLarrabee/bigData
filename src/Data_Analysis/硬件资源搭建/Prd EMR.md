...
现状：当前处理方式只能单次运行指定时间范围内的数据处理。因此需要一个

## 1.1. 任务调度

| **特点**       | **AWS Data Pipeline**       | **Apache Airflow**           | **AWS Lambda 和 CloudWatch Events** | **AWS Step Functions**       | **Apache NiFi**                      | **XXL-JOB**                | **EC2 上的 cron 任务**           |
| -------------- | --------------------------- | ---------------------------- | ----------------------------------- | ---------------------------- | ------------------------------------ | -------------------------- | -------------------------------- |
| **易用性**     | 配置复杂，适合 AWS 生态用户 | 配置灵活，需要一定学习曲线   | 易于设置，适合简单任务              | 配置较简单，图形界面易于理解 | 可视化界面，操作直观                 | 界面友好，配置简单         | 简单直接，适合小规模任务         |
| **集成性**     | 深度集成 AWS 服务           | 可扩展性强，支持多种外部工具 | 深度集成 AWS 服务                   | 深度集成 AWS 服务            | 可与多种系统集成，如 Kafka、数据库等 | 支持多种任务类型和外部系统 | 仅限于简单任务和 AWS 服务集成    |
| **可扩展性**   | 高                          | 高                           | 中                                  | 高                           | 高                                   | 高                         | 低                               |
| **监控和日志** | 内置监控和重试机制          | 丰富的监控和日志功能         | 基本的日志和监控功能                | 丰富的监控和日志功能         | 提供详细的监控和日志功能             | 提供任务日志和监控功能     | 需要手动设置日志和监控           |
| **成本**       | 成本较高                    | 开源免费，需要资源配置成本   | 成本低，按需计费                    | 成本中等，按使用量计费       | 开源免费，需要资源配置成本           | 开源免费，需要资源配置成本 | 低成本，按 EC2 和 EMR 使用量计费 |
| **实时性**     | 主要用于批处理，实时性较低  | 支持实时和批处理任务         | 适合低延迟的任务                    | 适合实时和批处理任务         | 强大的实时数据处理能力               | 支持定时和实时任务         | 主要用于定时任务，实时性较低     |
| **适用场景**   | 数据管道和批处理任务        | 复杂工作流和依赖任务         | 简单的任务调度和事件驱动任务        | 复杂任务编排和状态管理       | 实时数据流处理和复杂数据管道         | 分布式任务调度和批处理任务 | 简单的定时任务调度和管理         |
| **学习曲线**   | 陡峭                        | 中等                         | 平缓                                | 平缓                         | 平缓                                 | 平缓                       | 非常平缓                         |
...
核心需要解决的问题是能定时比如T处理T-1的数据。

![image-20240710111251215](img/posts/Prd EMR.asserts/image-20240710111251215.png)
