...
假设每天有10000个Get请求，年成本是1.46$

即只考虑存储成本的话，每个网络成本是0.5$每年。
...
重新计算压缩比，json格式可能消息冗余80%，也就是实际体积是5倍大，但是gzip压缩到0.042的，最后估算原始数据每天尺寸为7.88GB
...
当前Qoe亚太区33k网络，QPS两个接口分别是

kafka配置：定期清理1GB，尺寸大小1MB

网络AP平均数1.3

假设5ap-20sta场景，实际测量得到两个文件体积是26+54kb=80kb



设计：

数据上报15分钟1条，因此max-millis不建议超过此值

建议设置为60000即可

总数据量为33k*192=6.336m条

然后33k网路分4个consumer消费，则划归到每个pod的qps=18
